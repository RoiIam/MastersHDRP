                                        ...
                                        BSDFData bsdfData, float3 V)
{
    ...
    if ((light.lightDimmer > 0) && IsNonZeroBSDF(V, L, preLightData, bsdfData))
    {
        ...
        #endif
        {
            ...
            float NdotL = dot(bsdfData.normalWS, L);
            // No microshadowing when facing away from light (use for thin transmission as well)
            shadow *= NdotL >= 0.0
                          ? ComputeMicroShadowing(GetAmbientOcclusionForMicroShadowing(bsdfData), NdotL,
                                                  _MicroShadowOpacity)
                          : 1.0;
                                      ...
                                      LightData light, float3 L, float4 distances)
{
    ...
    float distBackFaceToLight = GetPunctualShadowClosestDistance(lightLoopContext.shadowContext, s_linear_clamp_sampler,
                                                                 posInput.positionWS, light.shadowIndex, L,
                                                                 light.positionRWS,
                                     ...
                                     BSDFData bsdfData, float3 V)
{
    ...
    if ((light.lightDimmer > 0) && IsNonZeroBSDF(V, L, preLightData, bsdfData))
    {
        ...
        #endif
        {
            ...
            SHADOW_TYPE shadow = EvaluateShadow_Punctual(lightLoopContext, shadowPositionInputs, light, builtinData,
                                                         GetNormalForShadowBias(bsdfData), L, distances);
            lightColor.rgb *= ComputeShadowColor(shadow, light.shadowTint, light.penumbraTint);
        ...
        {
            //render normally
            lighting = ShadeSurface_Infinitesimal(preLightData, bsdfData, V, L, lightColor.rgb, light.diffuseDimmer,
                                                  light.specularDimmer);
        }
        else
        {
            ...
            lighting = ShadeSurface_Infinitesimal_Glints(preLightData, bsdfData, input, V, L, lightColor.rgb,
                                                         light.diffuseDimmer, light.specularDimmer, toLocal,
