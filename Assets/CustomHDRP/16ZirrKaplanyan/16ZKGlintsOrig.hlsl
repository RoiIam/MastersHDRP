...
vec3 glints(vec2 texCO, vec2 duvdx, vec2 duvdy,
            mat3 ctf, vec3 lig, vec3 nor, vec3 view,
            vec2 roughness, vec2 microRoughness,
            float searchConeAngle, float variation, float dynamicRange, float density)
{
    ...
    // material
    vec2 mesoRoughness = sqrt(max(roughness * roughness - microRoughness * microRoughness, vec2(1.e-12)));
