    ...
    int mappingType, inout LayerTexCoord layerTexCoord)
{
    ...
    ADD_IDX(layerTexCoord.details).triplanarWeights = ADD_IDX(layerTexCoord.base).triplanarWeights = layerTexCoord.
        triplanarWeights;
...
float3 ADD_IDX(GetBentNormalTS)(FragInputs input, LayerTexCoord layerTexCoord, float3 normalTS, float3 detailNormalTS,
                                float detailMask)
{
    float3 bentNormalTS;
                              ...
                              out float3 normalTS, out float3 bentNormalTS)
{
    ...
    //surfaceData.baseColor = ADD_IDX(_MyColor).rgba; //RCC priklad zmeny textury
    //texture, sampler, (coordX,Y),index, pozor data su 1D a index sa meni, teda coordY bude vzdy 0, vsak?
    //pozri d3d11.hlsl keby nahodou
    //float1 x = 0.5;
    //float1 y = 0.5;
    float4 _sampledDict = SAMPLE_TEXTURE2D_ARRAY(_chSDFDict, sampler_chSDFDict, float2(0.5, 0), 0.5);
    //RCC podme sample dictionary tu bude frag riadok 258, musime to dat priamo do svetla
