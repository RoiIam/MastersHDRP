...
{
    private bool changeSimultaneously = true;
    ...
    public override void OnGUI(Rect position, MaterialProperty prop, GUIContent label, MaterialEditor editor)
    {
        if (prop.type == MaterialProperty.PropType.Vector)
        {
            ...
            var toggleRect = new Rect(position.x, position.y + 20, position.width + 20, 20);
            var vector2Rect = new Rect(position.x, position.y, position.width, 20);
            EditorGUI.BeginChangeCheck();
            var vec = EditorGUI.Vector2Field(vector2Rect, label, prop.vectorValue);
